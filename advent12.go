...
func main() {
	...
	exploreCave(connectionMap, "start", "")
...
func exploreCave(connectionMap map[string][]string, cave string, currentPath string) {
	for _, connectedCave := range connectionMap[cave] {
		...
		if connectedCave == strings.ToUpper(connectedCave) {
			exploreCave(connectionMap, connectedCave, currentPath+"->"+cave)
		} else {
